...
if nginx -s reload >/dev/null 2>&1; then :; else
...
for _ in 1 2 3 4 5; do
  if curl -fsS --max-time 4 "http://127.0.0.1:${PORT}/health" >/dev/null 2>&1; then LOCAL_OK=1; break; fi
  sleep 1
done
...
say ""
say "============================================"
if [ "$LOCAL_OK" -eq 1 ]; then say " [OK] TMS AI Router đã cài thành công!"; else
